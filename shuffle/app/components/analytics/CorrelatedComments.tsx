...
import type { AnalyticsFilters, Poll, Response } from "@/lib/api";
  ...
  >
    <div className="flex items-center justify-between">
      <div>
        ...
        <ChoiceBadge choice={commentAValence}>{commentAValence}</ChoiceBadge>
        on
      </div>
    </div>

    <div className="my-4 ml-3 text-sm italic text-gray-700 dark:text-gray-400">
      <span>{commentIdToCommentMap[commentA]?.text}</span>
    </div>

    <div className="mb-1">
      also voted{" "}
      <ChoiceBadge choice={commentBValence}>{commentBValence}</ChoiceBadge> on
    </div>

    <div className="my-4 ml-3 text-sm italic text-gray-700 dark:text-gray-400">
      <span>{commentIdToCommentMap[commentB]?.text}</span>
    </div>
  </li>
);

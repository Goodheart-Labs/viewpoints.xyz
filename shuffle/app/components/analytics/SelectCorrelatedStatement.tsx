...
type SelectCorrelatedStatementProps = {
  correlatedStatements: Correlation[];
  statementIdToStatementMap: Record<number, Statement>;
...
const SelectCorrelatedStatement = ({
  correlatedStatements,
  statementIdToStatementMap,
  onSelected,
  disabled = false,
}: SelectCorrelatedStatementProps) => {
  ...
  const filteredCorrelations =
    query === ""
      ? correlatedStatements
      : correlatedStatements.filter((correlation) => {
          const description = `${correlation.key} ${
            statementIdToStatementMap[correlation.statementA].text
          } ${statementIdToStatementMap[correlation.statementB].text}`;
                ...
                {({ active, selected }) => (
                  <>
                    <div className="flex flex-col">
                      ...
                      >
                        <ChoiceBadge choice={correlation.statementAChoice} />{" "}
                        {statementIdToStatementMap[correlation.statementA].text}
                      </span>
                      ...
                      >
                        <ChoiceBadge choice={correlation.statementBChoice} />{" "}
                        {statementIdToStatementMap[correlation.statementB].text}
                      </span>
                    </div>
...
export default SelectCorrelatedStatement;

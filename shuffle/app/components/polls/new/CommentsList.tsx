...
const CommentsList = ({
  ...
  const onClickAddComment = useCallback(
    () => setComments((oldComments) => oldComments.concat(newComment(""))),
    [],
  );
  ...
  const onClickRemoveComment = useCallback(
    (key: string) => {
      ...
      setComments((oldComments) => pullAllBy("key", [{ key }], oldComments));
    },
    [comments.length],
  );
  ...
  const onClickAutogenerate = useCallback(async () => {
    ...
    setComments((oldComments) => {
      ...
      const commentsValues = values(oldComments);
      ...
      return oldComments.concat(autogeneratedCommentsWithKeys);
    });
  ...
  const onChangeComment = useCallback(
    (key: string, comment: string) => [
      setComments((oldComments) =>
        set(
          oldComments.findIndex((c) => c.key === key),
          { key, comment },
          oldComments,
        ),
      ),
